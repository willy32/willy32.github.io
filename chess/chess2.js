...
let tileSize = {"x":boardSize.x / 5, "y":boardSize.y / 5}
    ...
    board.forEach((row, y) => {
        row.forEach((tile, x) => {
            if(tile == "wq"){
                ...
                // Checks along X + Y :
                if(x + 4 <= board[0].length && y + 4 <= board.length){
                    let checkwin = true;
                    for(let i = 1; i < 4; i++){
                        if(board[i + y][i + x] != tile){
                            checkwin = false;
                        }
                    }
                    if(checkwin) whiteWin = true;
